#!/usr/bin/env python3
"""CLI agent script to manage the web search task state machine.
...
from features.scanning.web_search_tasks import (
    generate_tasks,
    WEB_SEARCH_MUTABLE_FIELDS,
    WEB_SEARCH_METRIC_FIELDS,
    WEB_SEARCH_ALLOWED_METRICS,
)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Manage the web search task state machine."
    )
    parser.add_argument(
        "--action",
        type=str,
    ...
    parser.add_argument("--candidates-merged", type=int, default=0, help="Number of duplicate candidates that had new fields and were pushed as merge updates.")
    ...
    try:
        if args.action == "generate":
            ...
            merge_result = merge_existing_state(new_tasks, existing_tasks, WEB_SEARCH_MUTABLE_FIELDS)
            ...
            if not tasks:
                ...
                print(json.dumps(get_progress_summary([], WEB_SEARCH_METRIC_FIELDS, stale_timeout_minutes=args.stale_timeout_minutes)))
                return

            summary = get_progress_summary(tasks, WEB_SEARCH_METRIC_FIELDS, stale_timeout_minutes=args.stale_timeout_minutes)
            BackendObservability.info(
