...
3. Execute `python3 scripts/agent_social_search.py --city <CITY> --category <CATEGORY> --platform facebook --limit 10 --offset 0 --trace-id <CONVERSATION_ID>` to get the first page of candidate social page URLs (use the active Antigravity conversation ID for `--trace-id`).
4. For each candidate URL in the returned page:
   ...
   e. If verified as Filipino-affiliated, push to the database immediately to avoid context bloat. Do this by:
      ...
      ii. Executing the push command with the production flag and trace ID: `python3 scripts/agent_graphql_push.py --operation CreateListing --production --variables "$(cat tmp/fina_community_finder_payload_<timestamp>.json)" --trace-id <CONVERSATION_ID>`
...
7. Write a final status report to a markdown file in the `logs/` directory (e.g., `logs/fina_community_finder_report_YYYYMMDD_HHMM.md`). The report should use markdown tables for readability and include:
