...
2. Check the `logs/` directory to see if a report for this target city/category already exists with the exact same timestamp (e.g., `logs/fina_community_finder_report_YYYYMMDD_HHMM.md`). If an exact filename collision occurs, abort the run and report the collision. Otherwise, proceed automatically without asking for confirmation.
