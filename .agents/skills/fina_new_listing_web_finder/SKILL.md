...
4. For each candidate URL discovered in the search results:
   ...
   f. If verified as a NEW Filipino-affiliated listing, push to the database immediately to avoid context bloat. Do this by:
      ...
      with fields: name, category, city, description, address, latitude, longitude, facebookUrl/instagramUrl/tiktokUrl, facebookFollowers (Int), instagramFollowers (Int), tiktokFollowers (Int), sourceUrl, tags (MUST be a comma-separated string, e.g., "filipino,<category>,social-discovery", NOT a JSON array), verificationStatus ('UNVERIFIED'), status (use the status extracted in step 4.d).
