...
3. Read the `searchTemplates` list defined for the target `<CATEGORY>` inside `data/categories.json`. For each template, format it by replacing the `{city}` placeholder with `<CITY>` (e.g., `"Filipino restaurant in Sydney"`). Execute a hybrid search query pattern in three sequential rounds using these formatted templates:
   ...
   c. **Round 3 (General Web)**: Search the general web excluding the social domains: `<Formatted Template> -site:facebook.com -site:instagram.com` for each template to find independent websites, local blog reviews, and directory lists.
