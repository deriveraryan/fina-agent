...
2. Execute `python3 scripts/agent_maps_fetch.py --city <CITY> --category <CATEGORY> --limit 1 --offset 0 --trace-id <CONVERSATION_ID>` once to populate the local cache file: `.antigravity_saves/maps_cache_{city}_{category}.json` (where city is lowercase with underscores, and category is lowercase).
