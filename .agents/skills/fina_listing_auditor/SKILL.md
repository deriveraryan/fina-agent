...
1. Read the canonical category definitions and examples from `data/categories.json`.
2. Execute the fetch CLI script with the trace ID to retrieve listings:
   ```bash
   python3 scripts/agent_audit_listings.py --city <CITY> --limit 10 --offset 0 --trace-id <CONVERSATION_ID>
   ```
3. For each listing in the returned page:
...
5. If the returned JSON output indicates there are more listings (`has_more` is true), increment your offset by 10 and run step 2 again.
6. Once all listings are audited, write a consolidated run report in markdown to a file in the `logs/{YYYYMMDD}/` directory named:
