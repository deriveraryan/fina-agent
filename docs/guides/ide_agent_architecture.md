...
*   **Web Discovery**: Reads `data/categories.json` to locate available search templates for the `<CATEGORY>`, loads `.antigravity_saves/web_finder_session.json` to fetch and increment the last used index (modulo total templates), and uses the native web search tool to run queries using strictly this single formatted template across three search rounds (Facebook, Instagram, and General Web), avoiding redundant search templates.
