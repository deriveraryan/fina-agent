...
*   **Logic**:
    1. Generates all task permutations for a city (idempotent) via `scripts/agent_search_tasks.py --action generate --city <CITY>`, producing `data/listing_web_search_tasks_{city}.json` with all (category × template × location) combinations ordered by category, template index, and city-level first then suburbs from `data/top_suburbs_per_city.json`.
